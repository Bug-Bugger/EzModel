	...
	function handleWebSocketMessage(message: any) {
		...
		switch (message.type) {
			...
			case 'user_presence':
				...
				console.log('DEBUG: Raw active_users array:', message.data.active_users);
				console.log(
				...
				console.log('DEBUG: Setting connectedUsers to', activeUsers.length, 'users');
					...
					if (!isOwnMessage) {
						...
						if (tableNode) {
							const updatedFields = tableNode.data.fields.map((field) =>
								field.field_id === message.data.field_id ? { ...field, ...fieldUpdates } : field
							);
							flowStore.updateTableNode(tableNode.id, {
								fields: updatedFields
							});
