			...
			<div class="flex items-start justify-between">
				<div class="flex-1">
					<div class="flex items-center mb-4">
						<span class="text-3xl mr-3">{getDatabaseIcon($projectStore.currentProject.database_type)}</span>
						<div>
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
						<div class="metadata-item">
							...
							<span class="text-lg text-blue-600 font-medium">{$projectStore.currentProject.database_type}</span>
						</div>
						...
						<div class="metadata-item">
							...
							<span class="text-lg text-green-600 font-medium">{$projectStore.currentProject.tables?.length || 0}</span>
						</div>
						...
						<div class="metadata-item">
							...
							<span class="text-sm text-gray-600">{formatDate($projectStore.currentProject.updated_at)}</span>
						</div>
					</div>
				</div>
											...
											<div class="fields-preview">
												<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
													...
													{#if table.fields.length > 6}
														<div class="field-item bg-gray-100 px-3 py-2 rounded border text-sm text-gray-600 flex items-center justify-center">
						...
						{:else}
							<div class="empty-tables text-center py-8">
								<svg class="w-16 h-16 mx-auto mb-4 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2H5a2 2 0 00-2-2v2z" />
								</svg>
								<h3 class="text-lg font-medium text-gray-900 mb-2">No tables yet</h3>
								<p class="text-gray-600 mb-4">Start building your database schema by adding tables</p>
				...
				<div class="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
					...
					<div class="space-y-4">
						<div class="stat-item flex justify-between">
							...
							<span class="font-medium text-gray-900">{$projectStore.currentProject.tables?.length || 0}</span>
						</div>
						<div class="stat-item flex justify-between">
							<span class="text-gray-600">Total Fields</span>
							<span class="font-medium text-gray-900">
								{$projectStore.currentProject.tables?.reduce((total: number, table: any) => total + (table.fields?.length || 0), 0) || 0}
							</span>
						</div>
						<div class="stat-item flex justify-between">
							<span class="text-gray-600">Relationships</span>
							<span class="font-medium text-gray-900">{$projectStore.currentProject.relationships?.length || 0}</span>
						</div>
					</div>
				</div>
