	...
	{:else}
		<div class="projects-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
			{#each $projectStore.projects as project}
				<div
					class="project-card bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow"
				>
					<!-- Project Header -->
					...
					<div class="project-actions p-6">
						<div class="flex space-x-3">
							...
							<Button size="sm" onclick={() => editProject(project.id)} class="flex-1">
								{#snippet children()}
									<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
										<path
											stroke-linecap="round"
...
<Dialog bind:open={showCreateDialog} onOpenChange={closeCreateDialog}>
	<div class="space-y-4">
		<div>
			...
			<p class="text-sm text-gray-600">Set up a new database schema project</p>
		</div>
		...
		<div class="space-y-4">
			...
			<div class="space-y-2">
				...
				<Select bind:value={databaseType} options={databaseOptions} disabled={isCreating} />
			</div>
		</div>

		<div class="flex gap-2 pt-4">
			<Button variant="outline" class="flex-1" onclick={closeCreateDialog} disabled={isCreating}>
				Cancel
			</Button>
			<Button class="flex-1" onclick={createProject} disabled={isCreating || !projectName.trim()}>
				{#if isCreating}
