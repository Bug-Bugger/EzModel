...
<div class="collaboration-status flex items-center space-x-3">
	...
	<div class="flex items-center space-x-2">
		<div
			class="status-indicator px-2 py-1 rounded-full text-xs font-medium {getStatusColor($collaborationStore.connectionStatus)}"
		>
		...
		{#if $collaborationStore.lastError}
			<div class="error-message text-xs text-red-600 max-w-48 truncate" title={$collaborationStore.lastError}>
