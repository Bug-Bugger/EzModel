  ...
  return {
    ...
    async addTableNode(
      ...
      try {
        ...
        const backendTable: Table = await projectService.createTable(
          projectId,
          {
            name: table.name,
            pos_x: position.x,
    ...
    addLocalTableNode(
      table: Omit<TableNode["data"], "position">,
      position: Position
    ) {
      const newNode: TableNode = {
        id: table.table_id,
    ...
    updateLocalRelationshipEdge(
      edgeId: string,
      updates: Partial<RelationshipEdge["data"]>
    ) {
      update((state) => ({
        ...state,
        edges: state.edges.map((edge) =>
    ...
    async loadProjectRelationships(projectId: string): Promise<void> {
      try {
        const relationships = await projectService.getProjectRelationships(
          projectId
        );
        ...
        console.log(
          `Loaded ${relationships.length} relationships for project ${projectId}`
        );
      } catch (error) {
